            ...
            <table class="table table-hover text-nowrap">
                ...
                <tbody>
                    ...
                    @forelse ($datas as $produit)
                            @php($count++)
                            <form method="PUT" action="{{route('montant.update')}}" id="montant-produit_{{$produit->id}}">
                            <tr class="tr_{{$produit->id}}">
                                    ...
                                    <td><input type="text" value="{{ $produit->valeur_min }}" id="valeur_min_{{$produit->id}}" name="valeur_min_{{$produit->id}}" />
                                    </td>
                                    <td><input type="text" value="{{ $produit->valeur_max }}" id="valeur_max_{{$produit->id}}" name="valeur_max_{{$produit->id}}" />
                                    </td>
                                    @if ($produit->is_sale === 1)
